...
| **Tag mismatch**（SSL/TLS エラー、kotlin-compiler-embeddable 等のダウンロード失敗） | SSL/TLS 復号化の問題。**1)** Gradle キャッシュを完全にクリア: `Remove-Item -Recurse -Force "$env:USERPROFILE\.gradle\caches"` **2)** Gradle wrapper を再ダウンロード: `Remove-Item -Recurse -Force "$env:USERPROFILE\.gradle\wrapper\dists\gradle-8.14.3-bin"` **3)** ネットワーク接続を確認（VPN を無効化、プロキシ設定を確認） **4)** 時間を置いて再試行（Maven リポジトリ側の問題の可能性） |
